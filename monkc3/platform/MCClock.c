...
fun(bye, void)endfun as(MCClock)
...
fun(setTimeToNow, MCClock_t*)endfun as(MCClock)
	time_t timer = time(NULL);
	it->RawTime = *localtime(&timer);
    return it;
end

fun(setTime, void), struct MCTime time endfun as(MCClock)
...
fun(adjustTime, void), struct MCTime time endfun as(MCClock)
	if(time.tm_sec!=no_change) it->RawTime.tm_sec += time.tm_sec;
	if(time.tm_min!=no_change) it->RawTime.tm_min += time.tm_min;
	if(time.tm_hour!=no_change) it->RawTime.tm_hour += time.tm_hour;
	if(time.tm_mday!=no_change) it->RawTime.tm_mday += time.tm_mday;
...
fun(setRawTime, void), struct tm rawTime endfun as(MCClock)
	it->RawTime = rawTime;
end

fun(getTime, void), time_t* const result endfun as(MCClock)
	*result = mktime(&(it->RawTime));
end

fun(getRawTime, void), struct tm* const result endfun as(MCClock)
	*result = it->RawTime;
end

fun(getTimeByString, const char*)endfun as(MCClock)
	strcpy(it->currentTimeBuff, asctime(&(it->RawTime)));
	return it->currentTimeBuff;
end

fun(getCPUClocksPerSecond, void), clock_t* const result endfun is
	*result = CLOCKS_PER_SEC;
end

fun(getCPUClocksSinceStart, void), clock_t* const result endfun is
	*result = clock();
end

fun(getCPUSecondsSinceStart, void), time_t* const result endfun is
    *result = (clock() / CLOCKS_PER_SEC);
end

fun(getCPUSecondsSince, void), time_t since, time_t* const result endfun is
    *result = (clock() / CLOCKS_PER_SEC) - since;
end

fun(getCurrentTimeString, const char*)endfun as(MCClock)
	time_t timer = time(NULL);
	strcpy(it->currentTimeBuff, asctime(localtime(&timer)));
	return it->currentTimeBuff;
end

fun(getCurrentGMTTimeString, const char*)endfun as(MCClock)
	time_t timer = time(NULL);
	strcpy(it->currentGMTBuff, asctime(gmtime(&timer)));
	return it->currentGMTBuff;
end

fun(printTime, void)endfun as(MCClock)
	printf("\n%s", asctime(&it->RawTime));
end

fun(printCurrentTime, void)endfun
{
	time_t timer = time(NULL);
	printf("%s", asctime(localtime(&timer)));
}

fun(printCurrentGMTTime, void)endfun
{
	time_t timer = time(NULL);
