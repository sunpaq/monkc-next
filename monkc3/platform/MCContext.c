#include "MCContext.h"
#include "MCString.h"
#include "MCLog.h"

//Old Mac9 end of line sequence: \r
//Unix OSX end of line sequence: \n
//Windows  end of line sequence: \r\n
bool isNewLine(const char* s)
{
    if (s) {
        if (*s == '\n') {
            return true;
        } else if (*s == '\r') { //Windows NewLine
            return true;
        }
    }
    return false;
}

static char get_one_char(void)
{
    char cf = '\0';
    while(!isNewLine(&cf)) {
        cf = (char)getchar();
    };//clear the buff
    return cf;
}

static void get_chars_until_enter(char resultString[])
{
    char tc = NUL;
    int i=0;
    while(!isNewLine(&tc)){
        tc = (char)getchar();
        resultString[i]=tc;
        i++;
        putchar(tc);
    }
    resultString[i] = NUL;
    putchar(tc);
}

fun(bye, void) end_ as(MCContext)
    runtime_log("%s\n", "MCContext goodbye");
end

fun(dumpParas, void) end_ as(MCContext)
    int i;
    for (i = 0; i < it->argc; ++i) {
        runtime_log("%s\n", it->argv[i]);
    }
end

fun(getPara, char*), int index end_ as(MCContext)
    return it->argv[index];
end

fun(isIndexedParaEqualTo, int), int index, char* para end_ as(MCContext)
    char* para1 = it->argv[index];
    if (para1==null)return 0;
    if (strcmp(para1, para)==0)return 1;
    else return 0;
end

fun(isHavePara, int), char* para end_ as(MCContext)
    if(it==null)return 0;
    int i, res;
    for (i = 0; i < it->argc; ++i)
    {
        char* tmp = it->argv[i];
        if (tmp!=null && para!=null) {
            res = strcmp(tmp, para);
        } else {
            return 0;
        }
        if(res==0) {
            return 1;
        }
    }
    return 1;
end

fun(showMenuAndGetSelectionChar, char), int count, ... end_ as(MCContext)
    va_list ap;
    va_start(ap, count);
    int i;
    for (i = 0; i < count; ++i)
    {
        char* arg = va_arg(ap, char*);
        printf("%d.%s\n", i+1, arg);
    }
    printf("%s\n", "your selection is?");
    va_end(ap);
    it->selectionChar = get_one_char();
    return it->selectionChar;
end

fun(showConfirmAndGetBOOL, int), const char* confirm end_ is
    printf("%s (y/n)?\n", confirm);
    char cf = get_one_char();
    if (cf=='y'){
        return 1;
    }
    return 0;
end

fun(getUserInputString, void), char resultString[] end_ is
    get_chars_until_enter(resultString);
end

/*
#include <stdlib.h>
char *getenv(const char *name);
int putenv(char *str); //need "key=value"
int setenv(const char *name, const char *value, int rewrite);
int unsetenv(const char *name);
*/

fun(getEnvironmentVar, char*), const char* key end_ is
    //char *getenv(const char *name);
    return getenv(key);
end

fun(setEnvironmentVar, int), const char* key, const char* value, int isOverwrite end_ is
    //int setenv(const char *name, const char *value, int rewrite);
    if (setenv(key, value, isOverwrite)==0)
        return 0;
    else
        return -1;
end

fun(clearEnvironmentVar, int), const char* key end_ is
    //int unsetenv(const char *name);
    if (unsetenv(key)==0)
        return 0;
    else
        return -1;
end

constructor(MCContext), int argc, char** argv end_ is
    MCObject(any, "MCContext");
    as(MCContext)
        it->selectionChar = 0;
    end
    dynamic(MCContext)
        funbind(bye);
        funbind(dumpParas);
        funbind(getPara);
        funbind(isIndexedParaEqualTo);
        funbind(isHavePara);
        funbind(showMenuAndGetSelectionChar);
        funbind(showConfirmAndGetBOOL);
        funbind(getUserInputString);
        funbind(getEnvironmentVar);
        funbind(setEnvironmentVar);
        funbind(clearEnvironmentVar);
    end
    return any;
end
