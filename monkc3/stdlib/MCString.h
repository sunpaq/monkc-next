...
static inline char* MCStringFill(char* dest, const char* src) {
    char* res = strcpy(dest, src);
    dest[strlen(src)] = '\0';
    return res;
}

static inline bool MCStringEqualN(const char* A, const char* B, size_t n) {
    if (strncmp(A, B, n) == 0) {
        return true;
    }else{
        return false;
...
static inline bool MCStringEqual(const char* A, const char* B) {
    if (strcmp(A, B) == 0) {
        return true;
    }else{
        return false;
