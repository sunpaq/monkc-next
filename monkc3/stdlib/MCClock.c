...
fun(getCPUClocksPerSecond, void), clock_t* const result) as(MCClock)
	*result = CLOCKS_PER_SEC;
}

fun(getCPUClocksSinceStart, void), clock_t* const result) as(MCClock)
	*result = clock();
}

fun(getCPUSecondsSinceStart, void), time_t* const result) as(MCClock)
    *result = (clock() / CLOCKS_PER_SEC);
}

fun(getCPUSecondsSince, void), time_t since, time_t* const result) as(MCClock)
