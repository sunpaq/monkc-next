...
fun(createNode, struct TrieNode*), char byte) as(MCTrie)
...
fun(releaseNode, void), struct TrieNode* node) as(MCTrie)
    if (node) {
        int i;
        for (i=0; i<MCTrieWidth; i++)
            if (node->childs[i])
...
fun(keysWithPrefix, struct MCArray*), const char* prefix) as(MCTrie)
    struct MCArray* array = MCArray(alloc(MCArray), 10);
    return array;
}
